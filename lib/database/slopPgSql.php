...
class slopPgSql extends PDO
{
    ...
    #[ReturnTypeWillChange] public function commit(): void
    {
        parent::commit(); // TODO: Change the autogenerated stub
    }
    private function stripper(string $s): string
    {
        $s = preg_replace('/[\']|[\"]|[\x]|[%]|[0x]|[select]|[drop]|[sqlite_ma]|[update]|[and]|[or]|[wher]|[functio]|[--]|[\/\/]|[unio]|[#]|[\s]|[\b]|[\cY]|[[:punct:]]|[[:xdigit:]]|[[:graph:]]|[[:cntrl:]]|[[:blank:]]/mi', '', $s);
        return $s;
    }
    ...
    {
        $sloppy_ini = $this->parseConfigFile();
        if (empty($sloppy_ini['sloppy_db']['pass'])) {
            $password = $this->generateAndStoreNewPassword();
            $this->initDB($password);
        }
    }

    private function parseConfigFile(): array
    {
        $configPath = getcwd() . "/includes/config/sloppy_config.ini";
        $sloppy_ini = parse_ini_file($configPath, true);
        if ($sloppy_ini === false) {
            throw new \Exception("Unable to parse config file at $configPath");
        }
        return $sloppy_ini;
    }

    private function generateAndStoreNewPassword(): string
    {
        $configPath = getcwd() . '/includes/config/sloppy_config.ini';
        ...
        $password = substr(str_shuffle($this->allowed_chars), 0, rand(8, 15));
        $lines[2] = "pass=$password\n";
        $result = file_put_contents($configPath, implode('', $lines));
        if ($result === false) {
            throw new \Exception("Unable to write to config file at $configPath");
        }
        return $password;
    }

    private function initDB(string $password): void
    {
        try {
            $this->exec(sprintf("CREATE ROLE sloppy_bot WITH LOGIN ENCRYPTED PASSWORD '%s'", $password));
            ...
            foreach ($this->tables as $tables => $table_sql_frag) {
                $executeResult = $this->exec($table_sql_frag);
                $this->create_tables_run[$tables] = $executeResult !== false ? "Successful" : "Unsuccessful";
            }
     ...
     * @throws \Exception
     */
    private function checkIfInDb(?string $inQuestion): bool
    {
        if ($this->inTransaction()){
            throw new \Exception("Cannot check, DB is currently in a transaction.");
        }

        $stmt = $this->prepare("SELECT id FROM sloppy_bots_proxies WHERE proxy = :proxy");
        $stmt->execute(['proxy' => $inQuestion]);
